// ============================================================
//  SalesReturnPage.jsx  —  ↩️ مرتجعات المبيعات
...
const SAL_RETURN_LINE_COL_WIDTHS = ["4%", "28%", "11%", "12%", "12%", "20%", "9%", "4%"];
...
function ListScreen({ onNew }) {
  ...
  const { data, loading, refetch } = useApi(
    () => api.get("/returns", { params:{ type:"CUSTOMER", limit:100 } }), []
  );
  ...
  return (
    <AppLayout title="مرتجعات المبيعات" actions={<Button onClick={onNew}>✚ مرتجع جديد</Button>}>
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:".875rem" }}>
          <thead>
            <tr style={{ background:"var(--bg-surface)", borderBottom:"1px solid var(--border)" }}>
              {["رقم","التاريخ","الزبون","أصناف","الإجمالي","ملاحظة",""].map((h,i)=>(
      ...
      <div style={{ padding:"20px 22px" }}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:14, marginBottom:20 }}>
          {[
            ...
            { l:"الزبون",    v:data.PartyName, bold:true },
...
function FormScreen({ onDone }) {
  const { company } = useCompany();
  const logoUrl = companyLogoUrl(company?.CompanyInformation_Logo);
  const [hdr, setHdr] = useState({ Date_FRetern:today(), id_Zabon:"", Note_FRetern:"" });
  ...
  const { data: custBalData, loading: loadingBal } = useApi(
    () => hdr.id_Zabon ? partyService.getOneCustomer(hdr.id_Zabon) : Promise.resolve(null),
    [hdr.id_Zabon]
  );
  const custBal = custBalData?.data || null;
  // TotalDebt = صافي ديون الزبون — لا نستخدم NetBalance هنا لأنه يخصم المدفوعات/السماح مرة ثانية
  const prevBal = +(custBal?.TotalDebt ?? custBal?.totalDebt ?? 0);
  ...
  const getReturnPrice = async (mat) => {
    try {
      const r = await api.get("/returns/price-default", {
        params: { type: "CUSTOMER", id_Material_NoM: mat.id_Material_NoM, id_Party: hdr.id_Zabon || undefined },
      });
      return r2(r?.defaultPrice ?? r?.data?.defaultPrice ?? mat.LastSellPrice ?? mat.SellPrice1 ?? 0);
    } catch {
      return r2(mat.LastSellPrice || mat.SellPrice1 || 0);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await api.post("/returns", {
        ReturnType   : "CUSTOMER",
        id_Party     : hdr.id_Zabon,
        Date_FRetern : hdr.Date_FRetern,
        Note_FRetern : hdr.Note_FRetern,
        lines: validLines.map(l=>({ id_Material_NoM:l.id_Material_NoM, AmountReturn:+l.AmountReturn||1, PriceOUT:+l.PriceOUT||0, ReturnReason:l.ReturnReason||"" })),
      });
  ...
  if (savedId) {
    ...
    return (
    <AppLayout title="مرتجع مبيعات جديد" actions={<Button variant="ghost" onClick={onDone}>← رجوع</Button>}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14, minWidth:0, maxWidth:"100%", overflow:"hidden" }}>
          ...
          <Panel label="بيانات الإرجاع">
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:14 }}>
              ...
              <Fld label="الزبون *">
                <select value={hdr.id_Zabon} onChange={e=>sh("id_Zabon")(e.target.value)} style={selSt}>
            ...
            {hdr.id_Zabon && (
              <div style={{ marginTop:14, padding:"12px 14px", background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-md)", display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:12 }}>
                {loadingBal ? (
                  <div style={{ display:"flex", gap:8, color:"var(--text-muted)", fontSize:".84rem" }}><span className="spinner" style={{ width:14, height:14 }}/> جاري جلب الحساب...</div>
                ) : custBal ? (
                  <>
                    <SumInfo l="إجمالي الديون" v={fmtC(custBal.TotalDebt||0)} c="var(--warning)" />
                    <SumInfo l="المدفوع"        v={fmtC(custBal.TotalPaid||0)} c="var(--success)" />
                  </>
                ) : null}
              </div>
            )}
          ...
          <BarcodeScanner allMats={allMats} onAdd={async mat => {
            ...
            setLines(prev => {
              ...
              const d = { id_Material_NoM:mat.id_Material_NoM, MaterialName:mat.MaterialName, Barcode:mat.Barcode||"", Band:mat.Band||"", PriceOUT:price, stock:mat.QuantityOnHand||0 };
              if (empty>=0) return prev.map((l,i)=>i===empty?{...l,...d}:l);
              return [...prev,{...newLine(),...d}];
            });
          }}/>

          <Panel label={`أسطر الإرجاع (${validLines.length} صنف)`} noPad>
            <div style={{ overflowX:"auto", maxWidth:"100%", WebkitOverflowScrolling:"touch" }}>
              <table style={{ width:"100%", minWidth:640, tableLayout:"fixed", borderCollapse:"collapse", fontSize:".82rem" }}>
                <colgroup>
                  {SAL_RETURN_LINE_COL_WIDTHS.map((w, i) => (
                ...
                <tbody>
                  {lines.map((line,idx)=>(
                    <ReturnLineRow key={line._lid} idx={idx+1} line={line}
                ...
                <tfoot>
                  ...
                  {validLines.length > 0 && (
                    <tr style={{ borderTop:"2px solid var(--border)", background:"var(--bg-surface)" }}>
                      <td colSpan={6} style={{ padding:"10px", textAlign:"right", fontWeight:700, color:"var(--text-secondary)", fontSize:".82rem" }}>
                        إجمالي الأسطر ({validLines.length} صنف · {validLines.reduce((s,l)=>s+(+l.AmountReturn||0),0)} وحدة)
                      </td>
                      <td style={{ padding:"10px", textAlign:"left", fontFamily:"var(--font-mono)", fontWeight:900, color:"var(--accent)", fontSize:".95rem" }}>{fmtC(grandTotal)}</td>
                      <td/>
                    </tr>
                  )}
                </tfoot>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14, position:"sticky", top:72, minWidth:0, maxWidth:280 }}>
          <Panel label="ملخص الإرجاع">
            ...
            <div style={{ marginTop:14, paddingTop:14, borderTop:"1px dashed var(--border)" }}>
              <div style={{ fontSize:".68rem", fontWeight:700, color:"var(--text-muted)", textTransform:"uppercase", letterSpacing:".04em", marginBottom:10 }}>💳 حساب الزبون</div>
              {hdr.id_Zabon && custBal ? (
                <>
                  <SumRow l="الحساب السابق"  v={custBal.TotalDebt||0} c="var(--warning)"/>
                ...
                <div style={{ padding:"14px 10px", border:"1px dashed var(--border)", borderRadius:"var(--radius-sm)", textAlign:"center", fontSize:".8rem", color:"var(--text-muted)", lineHeight:1.7 }}>
                  اختر زبوناً<br/><span style={{ fontSize:".72rem" }}>لعرض الحساب</span>
                </div>
              )}
            </div>

            {grandTotal > 0 && (
              <div style={{ marginTop:10, padding:"10px 12px", background:"rgba(34,197,94,.08)", border:"1px solid rgba(34,197,94,.25)", borderRadius:"var(--radius-md)", fontSize:".77rem", color:"var(--accent)", lineHeight:1.9 }}>
                <b>الأثر عند الحفظ:</b><br/>
                📦 زيادة المخزون لكل صنف<br/>
                💳 خصم {fmtC(grandTotal)} من دين الزبون
              </div>
            )}
    ...
    <tr style={{ borderBottom:"1px solid var(--border-subtle)" }}
      ...
      <td style={{ padding:"8px 10px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:3, justifyContent:"center" }}>
          ...
          <input type="number" min="1" value={line.AmountReturn??1} onChange={e=>onUpdate({AmountReturn:Math.max(1,+e.target.value||1)})}
            style={{ ...numSt, width:44, textAlign:"center", padding:"4px 3px", fontWeight:800 }}/>
      ...
      <td style={{ padding:"8px 10px" }}>
        <input value={line.ReturnReason||""} onChange={e=>onUpdate({ReturnReason:e.target.value})} placeholder="اختياري..."
...
function BarcodeScanner({ allMats, onAdd }) {
  const [bc, setBc] = useState(""); const [msg, setMsg] = useState({text:"",ok:true}); const ref = useRef(null);
  const scan = v => {
    const val=(v||"").trim(); if(!val)return;
    const mat=allMats.find(m=>m.Barcode?.toLowerCase()===val.toLowerCase()||m.MaterialName?.toLowerCase().includes(val.toLowerCase()));
    if(mat){onAdd(mat);setBc("");setMsg({text:`✅ أُضيفت: ${mat.MaterialName}`,ok:true});ref.current?.focus();}
    else setMsg({text:`⚠ لا توجد مادة: ${val}`,ok:false});
    setTimeout(()=>setMsg({text:"",ok:true}),2500);
  };
  return (
    <Panel label="📷 مسح الباركود — اكتب أو امسح ثم اضغط Enter">
      <div style={{ display:"flex", gap:10, alignItems:"center" }}>
        <input ref={ref} value={bc} onChange={e=>setBc(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")scan(bc);}}
...
function SumRow({ l, v, c, accent, big, minus }) {
  return (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"5px 0", borderBottom:"1px solid var(--border-subtle)" }}>
      ...
      <span style={{ fontFamily:"var(--font-mono)", fontWeight:big?900:600, color:accent?"var(--accent)":c||"var(--text-primary)", fontSize:big?".95rem":".84rem" }}>{minus?"− ":""}{fmtC(v)}</span>
    </div>
  );
}
