...
const getMaterialsByCategory = async (req, res) => {
  try {
    ...
    const data = rows.map((r) => ({
      ...
      priceOptions    : [
        { label: "آخر سعر", value: r.LastSellPrice, key: "LastSellPrice" },
...
const searchMaterial = async (req, res) => {
  ...
  // ✅ FIX: دائماً ابحث بالباركود أولاً (نص)، ثم بالاسم، ثم بالـ ID كآخر خيار
  // السبب: الباركود أرقام طويلة مثل "6001001" كانت تُعامَل كـ ID وليس Barcode
  const isNumeric = /^\d+$/.test(identifier);

  try {
    // المرحلة 1: بحث بالباركود كنص — الأولوية دائماً
    let rows = await db.query(
      `SELECT
       ...
       WHERE CAST(m.Barcode AS TEXT) = CAST(? AS TEXT)
       ORDER BY m.MaterialName
       LIMIT 1`,
      [identifier]
    );

    // المرحلة 2: إذا لم يوجد بالباركود — بحث بالاسم
    if (!rows.length) {
      rows = await db.query(
        `SELECT
           m.id_Material_NoM, m.MaterialName, m.Barcode, m.Band,
    ...
    const data = rows.map((r) => ({
      ...
      priceOptions : [
        { label: "آخر سعر", value: r.LastSellPrice },
...
const checkout = async (req, res) => {
  ...
  const grandTotal = linesTotal - Number(Dis_FOUT);
  const prevBal    = await getZabonBalance(id_Zabon);
  ...
  try {
    ...
    if (payType.PayTypeName === "آجل") {
      ...
      await db.run(
        `INSERT INTO DionZabon_tbl
         ...
         VALUES (?, ?, ?, ?)`,
        [grandTotal, today, `POS فاتورة رقم ${invoiceId}`, id_Zabon]
      );
    }
    // نقدي → سيُعالَج مع وحدة الصندوق لاحقاً
    // (هنا نُسجّل فقط في AuditLog كإشعار)
    if (payType.PayTypeName === "نقدي" || payType.PayTypeName === "كاش") {
      await audit(
        req.user, "FOUT_tbl", invoiceId,
        "CashSale", null, grandTotal,
        `POS نقدي — ${grandTotal} | سيُضاف للصندوق`
      );
    }
    ...
    await audit(
      ...
      `POS | ${zabon.ZabonName} | ${payType.PayTypeName} | ${preparedLines.length} أصناف`
    );
    ...
    res.status(201).json({
      ...
      receipt          : {
        ...
        discount      : Number(Dis_FOUT),
        grandTotal    : Math.round(grandTotal * 100) / 100,
        previousBalance: prevBal.netBalance,
        newBalance    : payType.PayTypeName === "آجل"
          ? prevBal.netBalance + grandTotal
          : prevBal.netBalance,
